...
from ..combinatorics import combinations, permutations
...
def random_undirected(vertices=5, density=0.5, vw_range=(1, 1), el_range=(1, 1)):
    ...
    for source, target in random.sample([*combinations.combinations_itr([*range(vertices)], 2)], edges):
...
def random_directed(vertices=5, density=0.5, vw_range=(1, 1), el_range=(1, 1)):
    ...
    for source, target in random.sample([*permutations.permutations_cycle([*range(vertices)], 2)], edges):
