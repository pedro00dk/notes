...
def tsp_brute_force(
    ...
    - `start`: start vertex
    - `absent_edge_length`: length to use for absent edges, usually this algorithm is used in complete graphs, but when
        used as a hamiltonian path algorithm, this value must not be infinity, this allows finding paths with the
        minimal amount of absent edges
...
def tsp_nearest_heighbor(
    ...
    distance += min(edge.length for edge in graph.edges(path[-1]) if edge.target == start) if len(path) > 1 else 0
