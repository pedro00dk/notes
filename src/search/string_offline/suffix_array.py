...
class SuffixArray(StringOffline):
    ...
    def __init__(self, text: str, strategy: Literal['naive', 'kmr', 'kmr-opt'], lcp_strategy: Literal['naive', 'kasai']):
        """
    ...
    def _build_naive(self) -> list[int]:
        ...
        comparator: Callable[[int, int], int] = \
            lambda i, j: compare_slices(self._text, i, len(self._text), self._text, j, len(self._text))
    ...
    def _build_lcp_naive(self):
        ...
        """
        return [
            match_slices(self._text, self._suffixes[i], len(self._text),
                         self._text, self._suffixes[i + 1], len(self._text))
